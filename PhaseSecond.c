...
void encodingData(char *lineCheck, int *DC, FILE *dataFile) {
...
void encodingString(char *lineCheck, int *DC, FILE *dataFile) {
    int i;
    char restOfLine[MAX_LINE_LENGTH] = {0};
    copy(restOfLine, lineCheck);
...
void handleInstructions(GeneralList *symbolList, char *instruction, int *DC, FILE *dataFile) {
    if (!strcmp(".data", instruction))
        encodingData(strtok(NULL, ""), DC, dataFile);
    if (!strcmp(".string", instruction))
        encodingString(strtok(NULL, ""), DC, dataFile);
}

void encodingOneOperands(GeneralList *symbolList, char *command, int *IC, FILE *structionsFile, FILE *extFile) {
    ...
    if ((strcmp(command, "jmp") == 0) || (strcmp(command, "bne") == 0) ||
        ...
        if (firstOperand != NULL) {/*  if exist we will relate to it like jumpWithParametersAddress cass*/
            source = handelAddress(firstOperand, NULL, FALSE);
            secondOperand = strtok(NULL, " \t\n\v\f\r,)");/* cut the second parameter from the line */
            destination = handelAddress(secondOperand, NULL, FALSE);
        ...
        destination = handelAddress(operand, NULL, FALSE);
        /*encoding the first word for the command as a binary line from all relevant fields*/
...
void encodingTwoOperands(GeneralList *symbolList, char *command, int *IC, FILE *structionsFile, FILE *extFile) {
    int opCode = 0;/* the opCode of the command */
    addressingMode source = 0;/* the source address of the command */
    addressingMode destination = 0;/* the destination address of the command */
    char *firstOperand = NULL;
...
void handleImmediate(char *operand, int *IC, FILE *structionsFile) {
    char *end;
    int operandToInt = 0;
...
void handleDirect(GeneralList *symbolList, char *operand, int *IC, FILE *structionsFile, FILE *extFile) {
    int ARE = 2;
...
void handleAddress(addressingMode operandMode, GeneralList *symbolList, char *operand, int *IC,
    ...
    *IC = *IC + 1;
    if (operandMode == immediateAddress)
        handleImmediate(operand, IC, structionsFile);/* send it to be encoding the extra lines */
    else if (operandMode == directAddress)
        handleDirect(symbolList, operand, IC, structionsFile, extFile);/* send it to be encoding the extra lines */
    else {
...
void encodingSentence(GeneralList *symbolList, char *command, int *IC, FILE *structionsFile, FILE *extFile) {
    int opNumber;
    ...
    if (opNumber == 1)/*call method that will encode one operand command*/
        encodingOneOperands(symbolList, command, IC, structionsFile, extFile);
    if (opNumber == 2)/*call method that will encode two operand command*/
        encodingTwoOperands(symbolList, command, IC, structionsFile, extFile);
}

void endSecondPhase(char *fileName, GeneralList *symbolList, FILE *inputFile, FILE *entFile, FILE *extFile,
...
void finalFiles(char *fileName, FILE *structionsFile, FILE *dataFile) {
    char c;
...
void openFiles(char *fileName, FILE **inputFile, FILE **structionsFile, FILE **dataFile, FILE **extFile,
    ...
    *inputFile = openFile(fileName, "am", "r");/*open the received file for reading */
    *structionsFile = openFile(fileName, "cmd", "w+");/*open file to write all command sentence  */
    *dataFile = openFile(fileName, "data", "w+");/*open file to write all data and string sentence  */
    fprintf(*structionsFile, "\t%d\t%d\n", markValues[ICcounter] - 100,
            markValues[DCcounter]); /* write the final IC,DC on the file*/
}

void handleEntry(char *fileName, GeneralList *symbolList, FILE **entFile, int *markValues) {
...
int encodingSecondPass(char *fileName, GeneralList *symbolList, int *markValues) {
    ...
    handleEntry(fileName, symbolList, &entFile, markValues);/* handle entry file */
    while (!feof(inputFile)) {/*over all file, line by line*/
        ...
        if (curWord[0] == '.')  /* Check if it's an instruction (starting with '.') */
            handleInstructions(symbolList, curWord, &DC, dataFile);
        else /*it must be legal we check it in the PhaseOne*/
            encodingSentence(symbolList, curWord, &IC, structionsFile, extFile);
    }
    printf("Second pass has been finished successfully.\n");
    finalFiles(fileName, structionsFile, dataFile);/* combining 2 files in to final object file*/
    endSecondPhase(fileName, symbolList, inputFile, entFile, extFile, structionsFile,
