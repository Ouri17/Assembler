...
FILE *openFile(char *fileName, char *fileExtension, char *mode) {
    char fileNameWithExtension[MAX_FILE_NAME] = {0};
    FILE *fileToOpen;
    fullNameToFile(fileName, fileExtension, fileNameWithExtension);
    /*Opening the file with matching extension according to received mode*/
    fileToOpen = fopen(fileNameWithExtension, mode);
    return fileToOpen;
}

void removeFile(char *fileN, char *fileWithExtension) {
    char fileWithExtensionArray[MAX_FILE_NAME] = {0};
    fullNameToFile(fileN, fileWithExtension, fileWithExtensionArray);
    remove(fileWithExtensionArray);
}
...
void printError(char *errorMsg, int countLine) {
    printf("Error occurred at line %d: %s\n", countLine, errorMsg);
}

void printWarning(char *warningMsg, int countLine) {
...
int savedWord(char *word) {
    ...
    return (validRegName(word) || handleInstructionName(word) || validOpName(word));
}
