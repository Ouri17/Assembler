...
int handleInstructionName(char *instruction) {
    int i;
...
addressingMode handelAddress(char *operand, FILE *checksLabelFile, int flag) {
    int i = 0;
    if (operand != NULL && operand[0] != '\0') {
        ...
        if (handelLabelName(operand)) {/*when address is a valid label name,it's directAddressed*/
